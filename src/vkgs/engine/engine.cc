...
class Engine::Impl {
 ...
 public:
  Impl() {
    ...
    // creare pipeline layout to hold the push constants *************************
    ...
    // preallocate splat storage
    {
      ...
      splat_storage_.key = vk::Buffer(context_, MAX_SPLAT_COUNT * sizeof(uint32_t), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
      splat_storage_.index =
          vk::Buffer(context_, MAX_SPLAT_COUNT * sizeof(uint32_t), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);

      splat_storage_.cached_sorted_index_ = vk::Buffer(context_, MAX_SPLAT_COUNT * sizeof(uint32_t), VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
 ...
 private:
  ...
  void Draw() {
    ...
    // draw ui
    {
      ...
      ImGui::SliderInt("Sort Interval (frames)", &sort_interval, 1, 50);
    ...
    if (swapchain_.AcquireNextImage(image_acquired_semaphore, &image_index)) {
      ...
      if (loaded_point_count_ != 0) {
        ...
        // radix sort
        {
          ...
          if(sort_interval > 0 && (frame_counter_ % sort_interval == 0))
            vrdxCmdSortKeyValueIndirect(cb, sorter_, loaded_point_count_, splat_visible_point_count_, 0, splat_storage_.key, 0, splat_storage_.index, 0, sort_storage_, 0, NULL, 0);
          if (sort_interval > 0 && (frame_counter_ % sort_interval == 0)) {
            // Perform sort
            ...
            // After sorting, copy the sorted index to the cached buffer
            VkBufferCopy copy_region = {};
            copy_region.srcOffset = 0;
            copy_region.dstOffset = 0;
            copy_region.size = loaded_point_count_ * sizeof(uint32_t);
            vkCmdCopyBuffer(cb, splat_storage_.index, splat_storage_.cached_sorted_index_, 1, &copy_region);
          } else {
            // On non-sort frames, copy the cached sorted index back to the working index buffer
            VkBufferCopy copy_region = {};
            copy_region.srcOffset = 0;
            copy_region.dstOffset = 0;
            copy_region.size = loaded_point_count_ * sizeof(uint32_t);
            vkCmdCopyBuffer(cb, splat_storage_.cached_sorted_index_, splat_storage_.index, 1, &copy_region);
      }
        ...
        // draw
        {
          ...
          DrawNormalPass(cb, frame_index, swapchain_.width(), swapchain_.height(), swapchain_.image_view(image_index), alpha_threshold, splat_scale);
        ...
        DrawNormalPass(cb, frame_index, swapchain_.width(), swapchain_.height(), swapchain_.image_view(image_index), alpha_threshold, splat_scale);  // skip splat drawing by setting scale to 0
  ...
  void DrawNormalPass(VkCommandBuffer cb, uint32_t frame_index, uint32_t width, uint32_t height,
                      VkImageView target_image_view, float alpha_threshold, float splat_scale) {
    ...
    if (loaded_point_count_ != 0) {
      switch (splat_render_mode_) {
        case SplatRenderMode::TriangleList: {
         ...
         //SplatPushConstants push_constants = { alpha_threshold = 0.2,  splat_scale = 0.3f};
         glm::mat4 model(1.f);
         CombinedPushConstants push_constants = { model, alpha_threshold, splat_scale};
         vkCmdPushConstants(cb, graphics_pipeline_layout_, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(CombinedPushConstants), &push_constants);
  ...
  struct SplatStorage {
    ...
    vk::Buffer cached_sorted_index_;
  };
